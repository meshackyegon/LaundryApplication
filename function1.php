...
function laundry()
{
	...
	if(count($errors)==0){
    	...
    	if(in_array($filename, $allowTypes)){
...
function action(){
	global  $username,$email,$phoneNo, $pdate, $db, $filename, $typelaundry,$action;

	$username = e($_POST['username']);
	$email = e($_POST['email']);
	$phoneNo = e($_POST['phoneNo']);
	$sdate = e($_POST['sdate']);
	$laundrytype = e($_POST['laundrytype']);
	$filename=e($_POST["filename"] = !'');
	$pdate = e($_POST['pdate']);
	$action = e($_POST['action']);

	$sqll = "INSERT INTO action (username, email, phomeNo,sdate,laundrytype,filename,pdate, action)
	 VALUES ('$username', '$email', '$phoneNo', '$sdate', '$laundrytype', '$filename', '$pdate', '$action')";
